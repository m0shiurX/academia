<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <link rel="stylesheet" href="css/style.css">
    <title>Dashboard | Academia</title>
</head>
<body>
    <div class="container">
        <div class="top-bar">
            <h1 class="title">Academia | itGeeks </h1>
        </div>
        <div class="floating-menu minimal-menu">
            <div class="logo">
                <div class="box"></div>
            </div>
            <ul class="f-menu">
                <li><a href="dashboard.html">Dashboard</a></li>
                <li><a href="classroom.html">Class Room</a></li>
                <li><a href="teachers.html">Teacher</a></li>
                <li><a href="library.html">library</a></li>
                <li><a href="notebook.html">Notebook</a></li>
                <li><a href="others.html">Others</a></li>
            </ul>
        </div>
        <div class="menu-bar">
            <ul class="f-menu">
                <li><a href="semisterOne.html">First Semister</a></li>
                <li><a href="semisterOne.html">Second Semister</a></li>
                <li><a href="semisterOne.html">Third Semister</a></li>
                <li><a href="semisterOne.html">Fourth Semister</a></li>
                <li><a href="semisterOne.html">Fifth Semister</a></li>
                <li><a href="semisterOne.html">Sixth Semister</a></li>
                <li><a href="semisterOne.html">Seventh Semister</a></li>
                <li><a class="activated" href="#">Add New <span></span></a></li>
            </ul>
        </div>
        <main>
            <div class="objects">
                <p>Lorem ipsum dolor sit amet.</p>
            </div>
        </main>
        <div class="side-bar">
            <div class="top-sidebar">
                <div class="title">News Feed :</div>
            </div>
            <div class="newsfeed">
                <ul class="news">
                    <li>
                        <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Ad illum corrupti accusantium dolores incidunt nam!
                            <span>9 minute ago</span>
                        </p>
                        <div class="box"><img src="assets/prof.jpg" alt="" srcset=""></div>
                    </li>
                    <li>
                        <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Ad illum corrupti accusantium dolores incidunt nam!
                            <span>9 minute ago</span>
                        </p>
                        <div class="box"><img src="assets/prof.jpg" alt="" srcset=""></div>
                    </li>
                    <li>
                        <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Ad illum corrupti accusantium dolores incidunt nam!
                            <span>9 minute ago</span>
                        </p>
                        <div class="box"><img src="assets/prof.jpg" alt="" srcset=""></div>
                    </li>
                    <li>
                        <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Ad illum corrupti accusantium dolores incidunt nam!
                            <span>9 minute ago</span>
                        </p>
                        <div class="box"><img src="assets/prof.jpg" alt="" srcset=""></div>
                    </li>
                    <li>
                        <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Ad illum corrupti accusantium dolores incidunt nam!
                            <span>9 minute ago</span>
                        </p>
                        <div class="box"><img src="assets/prof.jpg" alt="" srcset=""></div>
                    </li>
                    <li>
                        <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Ad illum corrupti accusantium dolores incidunt nam!
                            <span>9 minute ago</span>
                        </p>
                        <div class="box"><img src="assets/prof.jpg" alt="" srcset=""></div>
                    </li>
                    <li>
                        <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Ad illum corrupti accusantium dolores incidunt nam!
                            <span>9 minute ago</span>
                        </p>
                        <div class="box"><img src="assets/prof.jpg" alt="" srcset=""></div>
                    </li>

                </ul>
            </div>
        </div>
    </div>
</body>
</html>